...
import { Eye, Pencil, Trash2, Search, CheckCircle, XCircle, Edit } from "lucide-react";
...
export default function AdminList() {
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      ...
      await api.put(`/webadmin/users/edit/${userId}`, updatedUser);
      setData((prev) =>
        prev.map((user) =>
  ...
  const fetchAdmins = async () => {
    try {
      setLoading(true);
      const response = await api.get("/webadmin/users/admins");
      setData(response.data);
    } catch (err) {
      console.error(err);
      if (err.response?.status === 403) {
        setIsPermissionDenied(true);    // 👈 set the flag
      } else {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex justify-center gap-3">
            <button
              onClick={() => navigate(`/webadmin/admin-view/${row.original.username}`)}
            ...
            <button
              onClick={() => navigate(`/webadmin/admin-edit/${row.original.username}`)}
  ...
  // ==============================================
  // ✅ RENDER FLOW – CHECK PERMISSION FIRST
  // ==============================================
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        ...
        <p className="text-lg font-semibold text-teal-600 animate-pulse">Loading Admins...</p>
      </div>
    );
  }

  // 🔥 If permission is denied, show AccessDenied page (full screen)
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="Delete Admin?"
        message="This action cannot be undone. Are you sure you want to delete this admin account?"
        confirmText="Delete"
        cancelText="Cancel"
      />
    </>
