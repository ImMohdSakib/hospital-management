...
export default function DoctorEdit() {
  ...
  // ===== Fetch doctor data and all users for uniqueness check =====
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);

        // 1. Fetch doctor data (using username as identifier)
        const doctorRes = await api.get(`/webadmin/users/${username}`);
        const doctorData = doctorRes.data;

        // 2. Fetch all doctors (for uniqueness check, excluding current)
        const allRes = await api.get('/webadmin/users/admins'); // We need a proper endpoint for all users, but using admins as fallback (should be fixed)
        const allUsers = allRes.data;

        const emails = allUsers
          .filter(user => user.id !== doctorData.id)
        ...
        if (doctorData.profilePhoto) {
          setProfilePhotoPreview(getImageUrl(doctorData.profilePhoto));
        }
      } catch (err) {
        console.error(err);
        setSubmitError(err.response?.data?.message || 'Failed to load doctor data.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Case 3: User didn't touch the photo at all -> keep existing profilePhoto
      // (already present in ...form)
      ...
      setSubmitError(err.response?.data?.message || 'Failed to update doctor. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && !form) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        ...
        <p className="text-lg font-semibold text-teal-600 animate-pulse">Loading Doctor Data...</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      >
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800">Edit Doctor</h2>
            <p className="text-sm text-slate-500">Update doctor account details</p>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {done ? (
            ...
            >
              ...
              <h3 className="mt-4 text-xl font-bold text-emerald-900">Doctor Updated Successfully!</h3>
            ...
            <motion.form key="form" onSubmit={handleSubmit} noValidate className="space-y-6">
              ...
              {/* ===== MEDICAL DETAILS (Doctor-specific) ===== */}
